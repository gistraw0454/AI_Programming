...
class Numeric(Problem):
    ...
    def takeStep(self, x, v):   # 기울기의 음의 방향으로 이동
        ...
        for i in range(len(x)): # x길이 만큼 반복
            newX[i] -= self._alpha * grad[i]   # 기울기 음의 방향으로 이동 
        
        if self.isLegal(newX):  # 범위가 타당한지 확인 하기 
            return newX
        return 
    ...
    def isLegal(self, x):   # low, up 추출
        ...
        for i in range(len(x)): # x길이 만큼 돌면서 범위내에 있는지 확인
            if not (low[i] <= x[i] <= up[i]):
                return False    # 범위 벗어나면?
        return True
    ...
    def gradient(self,x,v): # 해당부분에서의 기울기를 구한다.
        ...
        for i in range(len(x)):
            x_ = x[:]   # 복사
            x_[i] += self._dx # x_에 dx를 더해준다. !
            
            
            g = (self.evaluate(x_)-v)/self._dx  # 기울기 계산
...
class Tsp(Problem):
    def __init__(self):
        ...
        self._delta = 0.01 # 만들어야하나?

    def getDelta(self): # 만들어야하나?
        return self._delta
    ### Tsp methods
    ...
    def tenPerRow(self,solution):   # 그대로 
