...
class SimulatedAnnealing(MetaHeuristic): # MetaHeuristic 상속받는 SimulatedAnnealing class interface
    def __init__(self):
        MetaHeuristic.__init__(self)    # super class의 생성자 호출
...
class  HillClimbing(Optimizer):
    ...
    def setVariables(self, parameters): 
        ...
        Optimizer.setVariables(self,parameters)
        self._pType = parameters['pType']    # 디렉토리
...
class SimulatedAnnealing(MetaHeuristic):
    ...
    def run(self, p):
        ...
        while True:
            ...
            if t <= 0 or evaluations >= self._limitEval: 
                break  # 온도가 0이 되거나 최대 평가 횟수 초과 시 종료

            # 2. 현재 상태에 대한 randomMutant 생성
            mutant = p.randomMutant(current)

            # 3. Mutant 평가 (p.evaluate)
            valueM = p.evaluate(mutant)
            evaluations += 1

            f.write(str(valueC)+'\n')
            # 4. Mutant 값이 더 좋다면 update, 아니면 확률적으로 update
            if valueM < valueC:
                current, valueC = mutant, valueM
            else:
                dE = valueM - valueC  # 평가 값 차이
                if random.uniform(0, 1) < math.exp(-dE / t):  # 확률적으로 업데이트
                    current, valueC = mutant, valueM
