...
class Tsp(Problem):
    ...
    def oXover(self, chrInd1, chrInd2):   ### TODO : ppt 참고 구현하기 ! 찐 교차
        chr1 = chrInd1[:]
        chr2 = chrInd2[:]
        size = len(chr1)
        ...
        for i in range(size):
            if not holes1[temp1[(i+b+1)%size]]:
                ...
                k1 +=1
            if not holes2[temp1[(i+b+1)%size]]:
    ...
    def mutation(self, ind, mR):    # TODO : 주석 참고 구현하기 !
        ...
        if random.uniform(0, 1) <= mR:  # mR 변이확률에 따라 변이시키기
            # 1. 두 값 i, j (i < j) 생성
            i, j = sorted(random.randrange(self._numCities) for _ in range(2))  # 해당길이부터 2까지 무작위로 i,j를 뽑고 
